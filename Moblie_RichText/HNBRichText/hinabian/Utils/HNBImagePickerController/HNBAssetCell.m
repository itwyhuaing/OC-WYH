...
@implementation HNBAssetCell

// Now we use code to create subViews for improve performance
// 现在我们用代码来创建HNBAssetCell和HNBAlbumCell的子控件，以提高性能
...
- (void)setModel:(HNBAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[HNBImageManager manager] getAssetIdentifier:model.asset];
    }
    PHImageRequestID imageRequestID = [[HNBImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.hnb_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        // Set the cell's thumbnail image if it's still showing the same asset.
        if (!iOS8Later) {
            self.imageView.image = photo; return;
        }
        if ([self.representedAssetIdentifier isEqualToString:[[HNBImageManager manager] getAssetIdentifier:model.asset]]) {
    ...
    self.type = HNBAssetCellTypePhoto;
    if (model.type == HNBAssetModelMediaTypeLivePhoto)      self.type = HNBAssetCellTypeLivePhoto;
    else if (model.type == HNBAssetModelMediaTypeAudio)     self.type = HNBAssetCellTypeAudio;
    else if (model.type == HNBAssetModelMediaTypeVideo) {
        self.type = HNBAssetCellTypeVideo;
...
- (void)setNumForLabel:(NSInteger)num {
    self.selectView.numLabel.text = [NSString stringWithFormat:@"%lu",(unsigned long)num];
}
...
- (void)setType:(HNBAssetCellType)type {
    _type = type;
    if (type == HNBAssetCellTypePhoto || type == HNBAssetCellTypeLivePhoto) {
...
- (UIButton *)selectPhotoButton {
    if (_selectImageView == nil) {
        ...
        selectPhotoButton.frame = CGRectMake(self.hnb_width - 44, 0, 44, 44);
...
- (UIImageView *)imageView {
    if (_imageView == nil) {
        ...
        imageView.frame = CGRectMake(0, 0, self.hnb_width, self.hnb_height);
...
- (UIImageView *)selectImageView {
    if (_selectImageView == nil) {
        ...
        selectImageView.frame = CGRectMake(self.hnb_width - 20, 0, 20, 20);
...
- (UIView *)bottomView {
    if (_bottomView == nil) {
        ...
        bottomView.frame = CGRectMake(0, self.hnb_height - 17, self.hnb_width, 17);
...
- (UILabel *)timeLength {
    if (_timeLength == nil) {
        ...
        timeLength.frame = CGRectMake(self.viewImgView.hnb_right, 0, self.hnb_width - self.viewImgView.hnb_right - 5, 17);
...
- (RecentSelectView *)selectView {
    if (_selectView == nil) {
        float dis = 5.f;
        RecentSelectView *tempView = [[RecentSelectView alloc] initWithFrame:CGRectMake(self.bounds.size.width - 20 - dis, dis, 20, 20)];
        tempView.numLabel.hidden = YES;
        [tempView.numLabel setFont:[UIFont systemFontOfSize:12]];
...
- (void)setModel:(HNBAlbumModel *)model {
    _model = model;
    ...
    [[HNBImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.hnb_width - 24 - 30, 23, 24, 24);
}
...
- (UILabel *)titleLable {
    if (_titleLable == nil) {
        ...
        titleLable.frame = CGRectMake(80, 0, self.hnb_width - 80 - 50, self.hnb_height);
...
- (UIImageView *)arrowImageView {
    if (_arrowImageView == nil) {
        ...
        CGFloat arrowWH = 15;
        arrowImageView.frame = CGRectMake(self.hnb_width - arrowWH - 12, 28, arrowWH, arrowWH);
...
@implementation HNBAssetCameraCell
