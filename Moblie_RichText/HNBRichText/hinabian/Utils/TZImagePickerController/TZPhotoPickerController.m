...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,AlbumViewDelegate> {
...
//    _headView.backgroundColor = [UIColor greenColor];
    ...
    [_cancelBtn setTitle:[NSBundle tz_localizedStringForKey:@"Cancel"] forState:UIControlStateNormal];
...
//    _previewButton.tz_width = tzImagePickerVc.maxImagesCount <= 1 ? 0 : previewWidth + 2;
    _previewButton.tz_width = tzImagePickerVc.maxImagesCount <= 0 ? 0 : previewWidth + 2;
    ...
    if (_isIdeaBack) {
        ...
        _okButton.frame = CGRectMake(self.view.tz_width - 44 - 34, (50-22)/2, 66, 26);
        ...
        if (tzImagePickerVc.selectedModels.count > 0 || tzImagePickerVc.alwaysEnableDoneBtn) {
            ...
            [_okButton setTitle:[NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"OK(%zd)"],tzImagePickerVc.selectedModels.count] forState:UIControlStateNormal];
        }else {
            ...
            [_okButton setTitle:[NSBundle tz_localizedStringForKey:@"OK"] forState:UIControlStateNormal];
        }
...
- (void)okButtonClick {
    ...
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            [self didGetAllPhotos:photos models:tzImagePickerVc.selectedModels assets:assets infoArr:infoArr];
        }];
    }
...
- (void)didGetAllPhotos:(NSArray *)photos models:(NSArray *)models assets:(NSArray *)assets infoArr:(NSArray *)infoArr{
    ...
    if (tzImagePickerVc.didFinishPickingPhotosWithModelHanled) {
        tzImagePickerVc.didFinishPickingPhotosWithModelHanled(photos,models,assets,_isSelectOriginalPhoto);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_isIdeaBack) {
        ...
        cell.didSelectPhotoBlock = ^(BOOL isSelected) {
            ...
            if (isSelected) {
                ...
                for (TZAssetModel *model_item in selectedModels) {
                    if ([[[TZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[TZImageManager manager] getAssetIdentifier:model_item.asset]]) {
                        [tzImagePickerVc.selectedModels removeObject:model_item];
                        [tzImagePickerVc.selectedAssets removeObject:model_item.asset];
                ...
                if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                    ...
                    model.isSelected = YES;
                    [tzImagePickerVc.selectedModels addObject:model];
                    [tzImagePickerVc.selectedAssets addObject:model.asset];
                    [weakSelf refreshBottomToolBarStatus];
                    model.tag = [tzImagePickerVc.selectedModels indexOfObject:model] + 1;
                } else {
                    NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"Select a maximum of %zd photos"], tzImagePickerVc.maxImagesCount];
                    [tzImagePickerVc showAlertWithTitle:title];
                }
        ...
        if ([tzImagePickerVc.selectedModels indexOfObject:model] || [tzImagePickerVc.selectedModels indexOfObject:model] == 0) {
            NSInteger tempNum = [tzImagePickerVc.selectedModels indexOfObject:model] + 1;
...
//            }
            for (TZAssetModel *model in tzImagePickerVc.selectedModels) {
                if ([[[TZImageManager manager] getAssetIdentifier:tempCell.model.asset] isEqualToString:[[TZImageManager manager] getAssetIdentifier:model.asset]]) {
                    NSInteger tempNum = [tzImagePickerVc.selectedModels indexOfObject:model] + 1;
...
- (void)pushPhotoPrevireViewController:(TZPhotoPreviewController *)photoPreviewVc isPreview:(BOOL)isPreview {
    __weak typeof(self) weakSelf = self;
    
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (!_isIdeaBack) {
        if (isPreview) {
            photoPreviewVc.models = [NSMutableArray arrayWithArray:tzImagePickerVc.selectedModels];
        }else {
            photoPreviewVc.models = _models;
        }
    }
...
- (void)checkSelectedModels {
    if (_isIdeaBack) {
        ...
        for (TZAssetModel *model in _models) {
            ...
            //**重新赋值**
            for (TZAssetModel *model_select in tzImagePickerVc.selectedModels) {
...
//            }
            if (tzImagePickerVc.maxImagesCount <= 0) {
                [tzImagePickerVc.selectedModels addObject:assetModel];
                [self okButtonClick]; return;
            }
