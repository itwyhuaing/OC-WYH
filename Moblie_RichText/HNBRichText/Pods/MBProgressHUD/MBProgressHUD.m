...
- (void)updateConstraints {
    ...
    CGFloat y_offset = self.y_offset;
    ...
    CGSize maxmumSize = self.maxSize;
    if (!CGSizeEqualToSize(maxmumSize, CGSizeZero)) {
        NSMutableArray *maxSizeConstraints = [NSMutableArray array];
        [maxSizeConstraints addObject:[NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationLessThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:maxmumSize.width]];
        [maxSizeConstraints addObject:[NSLayoutConstraint constraintWithItem:bezel attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationLessThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:maxmumSize.height]];
        [self applyPriority:1000.f toConstraints:maxSizeConstraints];
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        if (idx == 0) {
            ...
            [bezelConstraints addObject:[NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:bezel attribute:NSLayoutAttributeTop multiplier:1.f constant:y_offset]]; //0.f
...
-(void)setActivityIndictorColor:(UIActivityIndicatorViewStyle)activityIndictorColor{
    
    if ([self.indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        UIActivityIndicatorView *tmp = (UIActivityIndicatorView *)self.indicator;
        tmp.activityIndicatorViewStyle = activityIndictorColor;
    }
