...
//                                });
    ...
    $(document).on('selectionchange',function(e){
        console.log("document.on&selectionchange");
...
//                 zss_editor.updateOffset();
    ...
    $(window).on('touchmove', function(e) {
        console.log("window.on&touchmove");
...
/***rain:监听标签点击事件*/
    ...
    $(window).on('touchend', function(e) {
                    if(!zss_editor.isDragging) {
                        if(e.target.id == "zss_editor_footer" || e.target.nodeName.toLowerCase() == "body" || e.target.nodeName.toLowerCase() == "html"){
                            console.log("捕获聚焦内容区域:"+e.target.id+','+e.target.nodeName);
                            zss_editor.focusContentEditor();
                        }
    ...
    $('#zss_editor_title').on('touchstart', function(e) {
                    if(!zss_editor.isDragging) {
                        console.log("捕获聚焦标题区域:"+e.target.id+','+e.target.nodeName);
                        window.webkit.messageHandlers.tapTitleSectionForEvent.postMessage(true);
                    }
                 });
    $('#zss_editor_content').on('touchstart', function(e) {
                        console.log("捕获聚焦内容区域:"+e.target.id+','+e.target.nodeName);
                        // send signal
                        window.webkit.messageHandlers.tapContentSectionForEvent.postMessage(true);
                    });
...
zss_editor.updateOffset = function() {
    console.log("zss_editor.updateOffset");
...
zss_editor.setScrollPosition = function() {
    ...
    console.log("zss_editor.setScrollPosition&window.pageYOffset="+position);
    /**rain - 输入过程中输入框抖动*/
...
zss_editor.calculateEditorHeightWithCaretPosition = function() {
    ...
    console.log("scrolltest.calculateEditorHeightWithCaretPosition&newPos="+newPos+',c='+c+',offsetY='+offsetY);
...
//}
// rain
zss_editor.insertImage = function(url, w,h,imgID,loadingURL,sg) {
    zss_editor.restorerange();
...
zss_editor.insertImageWithBase64StrAndLocalPath = function(imageBase64String, w,h,imgID,loadingURL) {
    zss_editor.restorerange();
    //var html = '<img id="'+imgID+'" src="'+url+'" width="'+w+'" height="'+h+'"/>';
    var prID = "prID"+imgID;
    var insertImageID = "insertImageID"+imgID;
...
// 图片资源全部设置为base64
zss_editor.insertImageWithBase64String = function(imageBase64String, w,h,imgID,loadingBase64String,sg) {
    zss_editor.restorerange();
    //var html = '<img id="'+imgID+'" src="'+url+'" width="'+w+'" height="'+h+'"/>';
    var prID = "prID"+imgID;
    var insertImageID = "insertImageID"+imgID;
    ...
    var extraBrTagID = "extraBrTagID"+imgID;
    var extraNbspTagID = "extraNbspTagID"+imgID;
    var html2 = '<div id="'+prID+'" class="pr appContent" contenteditable="false"><img id="'+insertImageID+'" class="app-img" src="data:image/jpeg;base64,'+imageBase64String+'" width="'+w+'" height="'+h+'"/><div id="'+maskID+'" class="pa paBox"><div class="com-table"><div contenteditable="false"><div class="grayMaskCL"><div class="com-table"><div class="table-cell"><img class="imgOperationCL" src="data:image/jpeg;base64,'+loadingBase64String+'"/><p class="textOperationCL uploadingMaskCL" contenteditable="false">上传中，请稍等...</p></div></div></div></div></div></div></div><div id="'+originTextID+'" class="imageOriginText" style="margin-left:'+sg+'px;margin-right:'+sg+'px;">上传中，请稍等...</div><div id="'+extraBrTagID+'" class="extraBrTagCL"><br></div><div id="'+extraNbspTagID+'" class="extraNbspTagCL">&nbsp;</div>'; //
...
zss_editor.focusContentEditor = function() {
...
zss_editor.getMainContentForEditor = function(){

    var cntDiv = document.getElementById("zss_editor_content");
    return cntDiv.innerHTML;
...
zss_editor.removeGrayMaskWithElementID = function(eleID){
    ...
    // V 3.1.0 不需要显示文本 ：图片来自海那边APP 。这里之所不直接删除的缘由：如果只上传图片，收起键盘则图片无法显示；故这里做隐藏，上传时移除
    zss_editor.hiddenHnbOriginText(originTextID);
...
zss_editor.reloadingImageGrayWithElementID = function(eleID,loadingBase64String){
    var maskID = "maskID"+eleID;
    var originTextID = "originTextID"+eleID;
    $("#"+maskID).unbind();
    document.getElementById(maskID).innerHTML = '<div class="com-table"><div contenteditable="false"><div class="grayMaskCL"><div class="com-table"><div class="table-cell"><img class="imgOperationCL" src="data:image/jpeg;base64,'+loadingBase64String+'"/><p class="textOperationCL uploadingMaskCL" contenteditable="false">上传中，请稍等...</p></div></div></div></div></div>';
...
zss_editor.queryImageMaskCountForCurrentDOM = function(){
    return $(".paBox").length;
} // end
...
zss_editor.getTidyedDOMAfterDelateHiddedHTML = function(){
    $("."+zss_editor.hiddedHTMLClass).remove();
    return zss_editor.getMainContentForEditor();
}
...
zss_editor.titleContentEditable = function(able){
    $("#zss_editor_title").attr("contenteditable",able);
} // end


zss_editor.hiddenHnbOriginText = function(textID){
    // 隐藏
    document.getElementById(textID).style.display = "none";
    // 添加标记 - 便于删除该标签
    document.getElementById(textID).setAttribute("class",zss_editor.hiddedHTMLClass);
    
}

zss_editor.removeHnbOriginText = function(textID){
    $("#"+textID).remove();
}
