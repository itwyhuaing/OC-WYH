...
@implementation HNBImageManager

static CGSize AssetGridThumbnailSize;
static CGFloat HNBScreenWidth;
static CGFloat HNBScreenScale;

+ (instancetype)manager {
    static HNBImageManager *manager;
    ...
    dispatch_once(&onceToken, ^{
        ...
        HNBScreenWidth = [UIScreen mainScreen].bounds.size.width;
        // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
        HNBScreenScale = 2.0;
        if (HNBScreenWidth > 700) {
            HNBScreenScale = 1.5;
        }
    });
    return manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (HNBScreenWidth - 2 * margin - 4) / columnNumber - margin;
    AssetGridThumbnailSize = CGSizeMake(itemWH * HNBScreenScale, itemWH * HNBScreenScale);
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(HNBAlbumModel *))completion{
    __block HNBAlbumModel *model;
    if (iOS8Later) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<HNBAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<HNBAssetModel *> *))completion {
    ...
    if ([result isKindOfClass:[PHFetchResult class]]) {
        ...
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (asset.mediaType == PHAssetMediaTypeVideo)      type = HNBAssetModelMediaTypeVideo;
            else if (asset.mediaType == PHAssetMediaTypeAudio) type = HNBAssetModelMediaTypeAudio;
            else if (asset.mediaType == PHAssetMediaTypeImage) {
                if (iOS9_1Later) {
                    // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = HNBAssetModelMediaTypeLivePhoto;
                }
            }
            if (!allowPickingVideo && type == HNBAssetModelMediaTypeVideo) return;
            if (!allowPickingImage && type == HNBAssetModelMediaTypePhoto) return;
            ...
            NSString *timeLength = type == HNBAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
            timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
            [photoArr addObject:[HNBAssetModel modelWithAsset:asset type:type timeLength:timeLength]];
        }];
...
#pragma clang diagnostic pop
            ...
            if (!allowPickingVideo){
                [photoArr addObject:[HNBAssetModel modelWithAsset:result type:type]];
                return;
            }
            /// Allow picking video
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                type = HNBAssetModelMediaTypeVideo;
                NSTimeInterval duration = [[result valueForProperty:ALAssetPropertyDuration] integerValue];
#pragma clang diagnostic pop
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr addObject:[HNBAssetModel modelWithAsset:result type:type timeLength:timeLength]];
            } else {
                if (self.hideWhenCanNotSelect) {
                    // 过滤掉尺寸不满足要求的图片
...
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(HNBAssetModel *))completion {
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        PHAsset *asset;
        @try {
        ...
        NSString *timeLength = type == HNBAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        HNBAssetModel *model = [HNBAssetModel modelWithAsset:asset type:type timeLength:timeLength];
        if (completion) completion(model);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
...
#pragma clang diagnostic pop
                ...
                HNBAssetModelMediaType type = HNBAssetModelMediaTypePhoto;
                if (!allowPickingVideo){
                    model = [HNBAssetModel modelWithAsset:result type:type];
                    if (completion) completion(model);
                    return;
                }
                /// Allow picking video
...
#pragma clang diagnostic pop
                    NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                    model = [HNBAssetModel modelWithAsset:result type:type timeLength:timeLength];
                } else {
                    model = [HNBAssetModel modelWithAsset:result type:type];
                }
                if (completion) completion(model);
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        HNBAssetModel *model = photos[i];
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if (model.type != HNBAssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            ...
            if (model.type != HNBAssetModelMediaTypeVideo) dataLength += (NSInteger)representation.size;
#pragma clang diagnostic pop
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes);
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = HNBScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        CGSize imageSize;
        if (photoWidth < HNBScreenWidth && photoWidth < _photoPreviewMaxWidth) {
            imageSize = AssetGridThumbnailSize;
        } else {
            PHAsset *phAsset = (PHAsset *)asset;
            CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
            CGFloat pixelWidth = photoWidth * HNBScreenScale;
...
/// Get postImage / 获取封面图
- (void)getPostImageWithAlbumModel:(HNBAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        id asset = [model.result lastObject];
        if (!self.sortAscendingByModificationDate) {
            asset = [model.result firstObject];
        }
        [[HNBImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (HNBAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    HNBAlbumModel *model = [[HNBAlbumModel alloc] init];
    model.result = result;
    model.name = name;
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
...
//@implementation HNBSortDescriptor
//
//- (id)reversedSortDescriptor {
//    return [NSNumber numberWithBool:![HNBImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
