...
@interface HNBVideoPlayerController () {
    AVPlayer *_player;
...
@implementation HNBVideoPlayerController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor blackColor];
    self.navigationItem.title = [NSBundle hnb_localizedStringForKey:@"Preview"];
    [self configMoviePlayer];
}

- (void)configMoviePlayer {
    [[HNBImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        _cover = photo;
    }];
    [[HNBImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)configPlayButton {
    ...
    _playButton.frame = CGRectMake(0, 64, self.view.hnb_width, self.view.hnb_height - 64 - 44);
